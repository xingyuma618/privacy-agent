...
class DatabaseManagerInterface:
    ...
    def query_lsh(self, keyword: str, signature_size: int, top_n: int) -> Dict[str, Dict[str, List[str]]]:
        raise NotImplementedError


class StandaloneInformationRetriever:
    """
    独立的信息检索模块，用于 NL2SQL 任务。
    整合了关键词提取、上下文检索和实体检索。
    """
    def __init__(self, db_manager: DatabaseManagerInterface, llm_keyword_extractor=None):
        """
        :param db_manager: 实现向量检索、LSH和Schema获取的数据库管理器
        :param llm_keyword_extractor: 一个可调用对象或函数，输入 (question, hint) 输出 List[str]（关键词列表）
        """
    ...
    def retrieve_information(self, question: str, hint: str, top_k: int = 5) -> Dict[str, Any]:
        """
        一键执行完整的信息检索流程。
        """
        # 1. 提取关键字
        keywords = self.extract_keywords(question, hint)
        logging.info(f"Extracted keywords: {keywords}")

        # 2. 检索相关上下文 (Context)
        schema_with_descriptions = self.retrieve_context(question, hint, keywords, top_k)

        # 3. 检索相关列和实体 (Entity)
        similar_columns = self._get_similar_columns(keywords, question, hint)
        schema_with_examples = self._get_similar_entities(keywords)
    ...
    # ==========================================
    def extract_keywords(self, question: str, hint: str) -> List[str]:
        if self.llm_keyword_extractor:
            return self.llm_keyword_extractor(question, hint)
        logging.warning("No LLM keyword extractor provided. Returning empty keywords.")
        return []

    # ==========================================
    # 2. Context Retrieval Logic
    # ==========================================
    def retrieve_context(self, question: str, evidence: str, keywords: List[str], top_k: int) -> Dict:
        logging.info("Finding the most similar columns for context")
        ...
        for keyword in keywords:
            ...
            retrieved_question_based_query = self.db_manager.query_vector_db(question_based_query, top_k=top_k)
            retrieved_evidence_based_query = self.db_manager.query_vector_db(evidence_based_query, top_k=top_k)
        ...
        return self._format_retrieved_descriptions(tables_with_descriptions)

    def _add_description(self, tables_with_descriptions: Dict, retrieved_descriptions: Dict) -> Dict:
    ...
    def _format_retrieved_descriptions(self, retrieved_columns: Dict) -> Dict:
    ...
    # ==========================================
    # 3. Entity & Column Retrieval Logic
    # ==========================================
    ...
    def _get_similar_column_names(self, keywords: List[str], question: str, hint: str) -> List[Tuple[str, str]]:
        ...
        schema = self.db_manager.get_db_schema()
        to_embed_strings = []
    ...
    def _get_to_search_values(self, keywords: List[str]) -> List[Dict[str, str]]:
    ...
    def _get_similar_entities_via_LSH(self, substring_packets: List[Dict[str, str]]) -> List[Dict[str, Any]]:
        ...
        for packet in substring_packets:
            ...
            unique_similar_values = self.db_manager.query_lsh(keyword=substring, signature_size=100, top_n=10)
