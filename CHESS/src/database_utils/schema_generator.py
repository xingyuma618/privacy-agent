...
MODEL_NAME = "/root/models/Qwen2.5-72B-Instruct"
# 从文本中提取被 ```sql 和 ``` 包裹的最后一段 SQL 代码，若没有则返回原文本。
def find_result(text, language='sql'):
    if '```sql' in text:
        pattern = r"```sql(.*?)```"
...
# 调用大模型，输出带有comment的schema
def openai_api(messages, temperature=0.1):
    # 这个函数中使用的client是进程内全局变量，请确保每个进程都初始化client
    while True:
        try:
...
def ddl_format(ddl):
    system_prompt = '你是一个SQLite数据库专家，请帮我把这个CREATE TABLE语句进行格式化（一行放一个列名或者主键、外键信息）,你还需要去掉CREATE TABLE语句中的注释，但不要修改表名、列名等原始结构，确保格式化的CREATE TABLE语句可以在SQLite数据库上运行。将结果放在```sql```markdown标签中'
    example_user = '''CREATE TABLE "club_rank" (
"Rank" real, /* Rank of the club */
"Club ID" int, -- id of the club
"Gold" real
, 
"Silver" real,"Bronze" real,
"Total" real,
PRIMARY KEY ("Rank",
"Club ID")
FOREIGN KEY (`Club ID`) REFERENCES `club`(`Club ID`)
)
'''
    example_assit = '''```sql
...
'''
    messages=[
        ...
        {"role": "assistant", "content": example_assit},
        {"role": "user", "content": ddl}
    ]
    resp = openai_api(messages)
    ddl = find_result(resp)
    return ddl
    ...
    def _extract_create_ddl_commands(self) -> Dict[str, str]:
        ...
        for table_name in self.schema_structure.tables.keys():
            create_prompt = execute_sql(db_path=self.db_path, 
                                        ...
                                        fetch="one")
            tmp_ddl = create_prompt[0] if create_prompt else ""

            tmp_ddl = ddl_format(tmp_ddl)


            ddl_commands[table_name] = tmp_ddl
        return ddl_commands
    ...
    def _get_example_column_name_description(self, table_name: str, column_name: str, include_value_description: bool = True) -> str:
        ...
        if column_info:
            if column_info.examples:
                ...
                example_part = f" Example Values: {', '.join([f'`{str(x)}`' if len(str(x))<256 else 'null' for x in column_info.examples])}"
        ...
        # if value_statics_string == "" and description_part.replace('\n','').strip() == "":
        #     joint_string = f" --{example_part} "
            ...
            for column_name, column_info in table_schema.columns.items():
                if with_keys or not (column_info.primary_key or column_info.foreign_keys or column_info.referenced_by):
                    ...
                    if column_info.examples:
                        ...
                        column_profile += f"Example of values in the column: {', '.join([f'`{str(x)}`' if len(str(x))<256 else 'null' for x in column_info.examples])}\n"
                    if column_info.primary_key:
