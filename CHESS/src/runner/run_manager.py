...
class RunManager:
    ...
    def worker(self, task: Task) -> Tuple[Any, str, int]:
        ...
        schema_str = system_state.get_schema_string(schema_type="tentative")
        return system_state, task.db_id, task.question_id, schema_str
    ...
    def task_done(self, log: Tuple[SystemState, str, int]):
        ...
        """
        state, db_id, question_id, schema_str = log
        if state is None:
            return
        ...
        # print(state.get_schema_string(schema_type="complete"))
        with open(self.args.sc_data_save_path, 'a', encoding='utf-8') as f:
            new_record = {
                'db_id': db_id,
